...
<div class="bg-gray-50">
    <main class="max-w-7xl mx-auto px-6 py-12">
        <header class="page-header mb-12">
            ...
            <?php the_archive_description( '<div class="archive-description text-xl text-gray-600 mt-4">', '</div>' ); ?>
        </header>
        ...
        <?php if ( have_posts() ) : ?>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                ...
                while ( have_posts() ) :
                    the_post();
                    ?>
                    <article id="post-<?php the_ID(); ?>" <?php post_class('bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden hover:shadow-lg transition-shadow duration-300'); ?>>
                        ...
                        <div class="p-6">
                            <header class="entry-header mb-4">
                                <?php the_title( sprintf( '<h2 class="entry-title text-2xl font-semibold text-secondary leading-tight"><a href="%s" rel="bookmark">', esc_url( get_permalink() ) ), '</a></h2>' ); ?>
                            </header>
        ...
        <?php else : ?>
            <section class="no-results not-found text-center py-16">
                <h2 class="text-2xl font-semibold text-secondary mb-4">Nothing Found</h2>
                <p class="text-gray-600">It seems we can&rsquo;t find what you&rsquo;re looking for. Perhaps searching can help.</p>
                <?php get_search_form(); ?>
            </section>
        <?php endif; ?>
    </main>
